...
from loss.qrregularizerloss import QRRegularizerLoss
...
class LitModel(pl.LightningModule):
    def __init__(self, cfg):
        ...
        if self.cfg.loss.name == "qrregularizer":
            self.orthogonal_loss = QRRegularizerLoss(cfg)
        else:
    ...
    def training_step(self, batch, batch_idx):
        ...
        channel_attention = model_outputs["channel_attention"]
        opl_loss = self.orthogonal_loss(channel_attention, y_true)
        loss = self.loss(y_pred.squeeze(), y_true) + opl_loss
        self.log("opl_loss", opl_loss.item(), on_epoch=True)
